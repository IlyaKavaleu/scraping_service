...
empty = "Unfortunately, there is no data available for your preferences."
...
if users_dct:
    ...
    qs = Vacancy.objects.filter(**params, timestamp=today).values()[0:90:3]
    vakancies = {}
    for i in qs:
        vakancies.setdefault((i['city_id'], i['language_id']), [])
        vakancies[(i['city_id'], i['language_id'])].append(i)
    for keys, emails in users_dct.items():
        rows = vakancies.get(keys, [])
        html = ''
        for row in rows:
